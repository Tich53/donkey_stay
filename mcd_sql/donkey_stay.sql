...
(
    ...
    (
        idcottage INTEGER PRIMARY KEY NOT NULL
        AUTO_INCREMENT,
    cottage_name VARCHAR
        (45) NOT NULL,
    cottage_region VARCHAR
        (255) NOT NULL,
    cottage_city VARCHAR
        (45) NOT NULL,
    cottage_country VARCHAR
        (45) NOT NULL,
    ...
    cottage_photo6 VARCHAR
        ...
        (
            idoptional INTEGER PRIMARY KEY NOT NULL
            AUTO_INCREMENT,
    ...
    optional_price_per_child FLOAT NOT NULL
            ...
            (
                idbooking INTEGER PRIMARY KEY NOT NULL
                AUTO_INCREMENT,
    ...
    CONSTRAINT fk_booking_user FOREIGN KEY
                (user_iduser) REFERENCES user
                (iduser),
    cottage_idcottage INTEGER NOT NULL,
    CONSTRAINT fk_booking_cottage FOREIGN KEY
                (cottage_idcottage) REFERENCES cottage
                (idcottage),
    optional_idoptional INTEGER NOT NULL,
    CONSTRAINT fk_booking_optional FOREIGN KEY
                (optional_idoptional) REFERENCES optional
                (idoptional)
);

                CREATE TABLE booked_date
                (
                    idbooked_date INTEGER PRIMARY KEY NOT NULL
                    AUTO_INCREMENT,
    ...
    end_booked_date DATE NOT NULL
                    ...
                    (
                        idcottage_has_booked_date INTEGER PRIMARY KEY NOT NULL
                        AUTO_INCREMENT,
    cottage_idcottage INTEGER NOT NULL,
    CONSTRAINT fk_cottage_has_booked_date_cottage_cottage FOREIGN KEY
                        (cottage_idcottage) REFERENCES cottage
                        (idcottage),
    booked_date_idbooked_date INTEGER NOT NULL,
    CONSTRAINT fk_cottage_has_booked_date_booked_date FOREIGN KEY
                        (booked_date_idbooked_date) REFERENCES booked_date
                        (idbooked_date)
);

                        /* Insertion données table user */
                        INSERT INTO user
                            (username, user_password, user_firstname, user_lastname, user_phone, user_email)
                        VALUES
                            ('Tich53', 'richard123', 'Richard', 'Douetté', '0695936812', 'richard.douette-std@donkey.school');

                        INSERT INTO user
                            (username, user_password, user_firstname, user_lastname, user_phone, user_email)
                        VALUES
                            ('PatrickT', 'patrick123', 'Patrick', 'Tanguy', '06', 'patrick.tanguy-std@donkey.school');

                        INSERT INTO user
                            (username, user_password, user_firstname, user_lastname, user_phone, user_email)
                        VALUES
                            ('Khouloud', 'khouloud123', 'Khouloud', 'Hadhek', '06', 'khouloud.hadhek-std@donkey.school');

                        INSERT INTO user
                            (username, user_password, user_firstname, user_lastname, user_phone, user_email)
                        VALUES
                            ('Daniel', 'daniel123', 'Daniel', 'DESCLOUX', '06', 'daniel.descloux-std@donkey.school');


                        /* Insertion données table optional */
                        INSERT INTO optional
                            (optional_name, optional_price_per_adult, optional_price_per_child)
                        VALUES
                            ('Donkey ride', 12, 7);

                        /* Insertion données table cottage */
                        INSERT INTO cottage
                            (cottage_name, cottage_region, cottage_city, cottage_country, cottage_nb_bed, cottage_nb_bathroom, cottage_price_per_night, cottage_photo1, cottage_description)
                        VALUES
                            ('MAISON MÉDIÉVALE', 'Catalogne', 'El Canós', 'Espagne', 3, 2, 120, 'images/gite1_1.webp', "Casa del Portal est une maison privée, située dans un petit village médiéval (El Canós), entouré de champs, dans la région de Segarra (Lleida). Entièrement restauré, il dispose de 3 chambres d'une capacité de 8 personnes, d'un salon avec cheminée, d'une salle à manger, d'une cuisine, d'une salle de bain avec baignoire et terrasse. Dans le village, il y a une piscine avec une aire de jeux, que les clients peuvent utiliser gratuitement."),
